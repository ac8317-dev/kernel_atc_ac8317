 ...
 * Will only add unmatched mode entries
 ...
 fb_var_to_videomode函数只是将fb_var_screeninfo结构转换成fb_videomode结构。
 而fb_add_videomode函数查找是否该fb_videomode已经存在，如果不存在则添加到列表中。
...
int fb_add_videomode(const struct fb_videomode *mode, struct list_head *head)
{
	...
	list_for_each(pos, head) {
		modelist = list_entry(pos, struct fb_modelist, list);    /*遍历所有的fb_modelist，查找mode是否存在*/ 
		m = &modelist->mode;
