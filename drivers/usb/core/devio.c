...
int __init usb_devio_init(void)
{
	...
	retval = register_chrdev_region(USB_DEVICE_DEV, USB_DEVICE_MAX,     //分配设备号  
					"usb_device");
	if (retval) {
		printk(KERN_ERR "Unable to register minors for usb_device\n");
		goto out;
	}
	cdev_init(&usb_device_cdev, &usbdev_file_operations);                 //字符设备初始化    向vfs层提供的操作函数集  
...
#endif
	usb_register_notify(&usbdev_nb);     //注册设备通知链   usb通知链表头为usb_notifier_list  
