...
#define APPENDNUMBER 3 	// 2
				...
				stream->cur_format->fcc == V4L2_PIX_FMT_MJPEG )
			{
				...
				if((mem[0] == 0xFF) && (mem[1] == 0xD8))
				{
                    ...
                    /* Added by Joshua Wang.  @30/07/2015  WORKAROUND. +++++++++
                     * Some mjpeg is failed beacause two header info is included.
                     * So the first header is ticked off.
                     */
					for(i = 2; i < 0x500; i ++)
					{
                        if((mem[i] == 0xFF) && (mem[i+1] == 0xD8))
                        {
                            index = i;

                            if( (width == (*(mem+index+9)<<8)|(*(mem+index+10)))
                                && (height == (*(mem+index+7)<<8)|(*(mem+index+8))))
                            {
                                marker = index;
                                uvc_trace(UVC_TRACE_FRAME, "Sorry,The 1# Header of MJPG will be deleted.\n");
					...
					if(marker)
					{
						memcpy(buf->mem, mem+marker, buf->bytesused-marker);
					}
                    /* Added by Joshua Wang.   @30/07/2015 WORKAROUND.  ++++++++++++             */
...
static void uvc_video_complete(struct urb *urb)
{
	...
	/****not submit urb to usb when the urb was been killed ******/
   	if (atomic_read(&urb->reject)){
		uvc_printk(KERN_INFO, "urb->reject = %d\n",urb->reject);
		return;
	}
...
{
	unsigned int i;
	enum usb_device_state uvc_state;
	uvc_state = stream->dev->udev->bus->root_hub->state;
	for (i = 0; i < UVC_URBS; ++i) {
		if (stream->urb_buffer[i]) {

#ifndef CONFIG_DMA_NONCOHERENT
			if(uvc_state == USB_STATE_CONFIGURED){
...
int uvc_video_enable(struct uvc_streaming *stream, int enable)
{
	...
	if (!enable) {
		...
		uvc_printk(KERN_INFO, "uvc_video_enable+++[0x%x][%d].\n", stream, enable);
		return 0;
	}
