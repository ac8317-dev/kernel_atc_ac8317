...
static __devinit int wm8994_device_init(struct wm8994 *wm8994, int irq)
{
	...
	ret = mfd_add_devices(wm8994->dev, -1,                //  platform_device_alloc
			      wm8994_devs, ARRAY_SIZE(wm8994_devs),
			      NULL, 0);                                       //   xl  添加 codec子设备 和 gpio子设备 
	if (ret != 0) {
		dev_err(wm8994->dev, "Failed to add children: %d\n", ret);
		goto err_irq;
	}
				      ...
				      const struct i2c_device_id *id)
{
	...
	wm8994->regmap = devm_regmap_init_i2c(i2c, &wm8994_base_regmap_config);  //   xl  map  
	...
	return wm8994_device_init(wm8994, i2c->irq);   // xl add  mfd  sub devices  
}
