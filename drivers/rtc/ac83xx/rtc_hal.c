...
int RTCIsTimeRight(void)
{
	...
	value = RTCYearRead();
	printk(KERN_INFO "rtc: year = %d \n", value);
	if (!(value >= 0 && value <= 127)) {
		//printk(KERN_INFO "rtc: year ERROR \n");
		return -1;
	}

	value = RTCMonthRead();
	printk(KERN_INFO "rtc: Month = %d \n", value);
	if (!(value >= 1 && value <= 12)) {
		//printk(KERN_INFO "rtc: Month ERROR \n");
		return -1;
	}

	value = RTCDOMRead();
	printk(KERN_INFO "rtc: Day = %d \n", value);
	if (!(value >= 0 && value <= 31)) {
		//printk(KERN_INFO "rtc: Day ERROR \n");
		return -1;
	}

	value = RTCHourRead();
	printk(KERN_INFO "rtc: Hour = %d \n", value);
	if (!(value >= 0 && value <= 23)) {
		//printk(KERN_INFO "rtc: Hour ERROR \n");
		return -1;
	}

	value = RTCMinRead();
	printk(KERN_INFO "rtc: Min = %d \n", value);
	if (!(value >= 0 && value <= 59)) {
		//printk(KERN_INFO "rtc: Min ERROR \n");
		return -1;
	}

	value = RTCSecRead();
	printk(KERN_INFO "rtc: Sec = %d \n", value);
	if (!(value >= 0 && value <= 59)) {
		//printk(KERN_INFO "rtc: Sec ERROR \n");
		return -1;
	}

	printk(KERN_INFO "rtc: time is RIGHT \n");
	return 0;
...
{	
	...
	printk(KERN_INFO "RTCHWInit -> start \n");
	...
	for(i = 0;i < RTCINTMAX; i++) 
	{
		while (RTC_READ32(RTC_BBPU) & 0x80)
		{
			...
			if (count > 100) {
				count = 0;
				printk(KERN_INFO "RTCHWInit -> can not read RTC reg, pls check rtc clock.\n");
				break;
	  	...
	  	WAITBUSY();	
	    printk(KERN_INFO "RTCHWInit -> rtc is stable.\n");
	  	RTCProtect(0);
	  	...
	  	if(0x43 != (uint16_t)(RTC_READ32(RTC_BBPU) >> 8))
	  	{
 			RTC_WRITE32(RTC_BBPU,(RTC_READ32(RTC_BBPU) | 0x4302));  //bbpu bit[1] write_en
	  		CORETORTC();
			printk(KERN_INFO "RTCHWInit -> rtc is ok to do bus write.\n");
	  	}
		RTCProtect(1);
		...
		{
			printk(KERN_INFO "RTCHWInit -> POWERKEY is fit %d times.\n",(i + 1));
			ret = TRUE;
			break;		
		}

		printk(KERN_INFO "RTCHWInit -> POWERKEY is not fit %d times.\n",(i + 1));
		if ( (i+1) == (RTCINTMAX-1) ) {
			printk("RTCHWInit -> start to reset RTC ! \n");
			if((0xa357 != (uint16_t)RTC_READ32(RTC_POWERKEY1)) || (0x67d2 != (uint16_t)RTC_READ32(RTC_POWERKEY2)))
			{
				if (RTCIsTimeRight() != 0) {
					RTCProtect(0);
					RtcFirstRunChk();
					RTCProtect(1);
					printk("RTCHWInit -> set RTC powerkey and value !\n");
				} else {
					RTCProtect(0);
					RtcFirstSetPowerkeys();
					RTCProtect(1);
					printk("RTCHWInit -> set RTC powerkey ! \n");
				}
				ret = FALSE;
			}
		}
	...
	if (ret == FALSE ) {
		printk("RTCHWInit -> RTC init wrong !\n");
		g_bInitOK = FALSE;
	} else if (ret == TRUE) {
		printk("RTCHWInit -> RTC init OK ! \n");
		g_bInitOK = TRUE;
	}
