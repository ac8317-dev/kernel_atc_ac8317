				    ...
				    struct vm_area_struct *vma)
{
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		*page = alloc_page(GFP_KERNEL | __GFP_HIGHMEM | __GFP_ZERO);   // 真正进行内存的分配    
		if (*page == NULL) {
		...
		ret = map_vm_area(&tmp_area, PAGE_KERNEL, &page_array_ptr);  //为内核的这段虚拟地址建立虚拟到物理页的映射     // 在内核空间进行内存映射	   
		if (ret) {
			printk(KERN_ERR "binder: %d: binder_alloc_buf failed "
...
static int binder_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	area = get_vm_area(vma->vm_end - vma->vm_start, VM_IOREMAP);   // 在内核空间获取一块地址范围  
	if (area == NULL) {
		ret = -ENOMEM;
	...
	proc->user_buffer_offset = vma->vm_start - (uintptr_t)proc->buffer;   // 记录内核空间与用户空间的地址偏移  
	...
	// 通过下面这个函数真正完成内存的申请和地址的映射	// 初次使用，先申请一个PAGE_SIZE大小的内存
	if (binder_update_page_range(proc, 1, proc->buffer, proc->buffer + PAGE_SIZE, vma)) {
		ret = -ENOMEM;
		failure_string = "alloc small buf";
		goto err_alloc_small_buf_failed;
	}
	buffer = proc->buffer;    // proc->buffers用来连接所有已建立映射的binder内存块   
...
// 创建进程对应的binder_proc对象
	...
	init_waitqueue_head(&proc->wait);// 初始化binder_proc
	proc->default_priority = task_nice(current);
 
	binder_lock(__func__);   // 锁保护   
...
// 这里除了owner之外，每一个字段都是一个函数指针，这些函数指针对应了用户空间在使用Binder设备时的操作  
static const struct file_operations binder_fops = {
	.owner = THIS_MODULE,
	.poll = binder_poll,
	.unlocked_ioctl = binder_ioctl,//  这其中，有三个函数尤为重要，它们是：binder_open，binder_mmap和binder_ioctl。 这是因为，  
	.mmap = binder_mmap, //   需要使用Binder的进程，几乎总是先通过binder_open打开Binder设备，然后通过binder_mmap进行内存映射。
	.open = binder_open, //  在这之后，通过binder_ioctl来进行实际的操作。Client对于Server端的请求，以及Server对于Client请求结果的返回，都是通过ioctl完成的。
	.flush = binder_flush,
	.release = binder_release,
};
static struct miscdevice binder_miscdev = {   //  内核中注册了Binder设备
	.minor = MISC_DYNAMIC_MINOR,
	.name = "binder",     //  这里指定了Binder设备的名称是“binder”。这样，在用户空间便可以通过对/dev/binder文件进行操作来使用Binder  
	.fops = &binder_fops  //  binder_miscdev同时也指定了该设备的fops  
};
