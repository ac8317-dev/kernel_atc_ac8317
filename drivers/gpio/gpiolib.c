...
int gpiochip_add(struct gpio_chip *chip)
{
	...
	unsigned	id;
	int		base = chip->base;
    printk(KERN_ERR "[gpiolib]----->gpiochip_add chip->base=%d,chip->ngpio=%d<-----\n",chip->base,chip->ngpio);
...
int gpio_request(unsigned gpio, const char *label)
{
	...
	int			status = -EINVAL;
	unsigned long		flags;
     printk(KERN_ERR "[gpiolib]----->gpio_request gpio=%d,label=%s<-----\n",gpio,label);
	spin_lock_irqsave(&gpio_lock, flags);
	...
	if (test_and_set_bit(FLAG_REQUESTED, &desc->flags) == 0) {
		...
		status = 0;
		printk(KERN_ERR "[gpiolib]----->gpio_request FLAG_REQUESTED is not set <-----\n");
	} else {
	   printk(KERN_ERR "[gpiolib]----->gpio_request FLAG_REQUESTED is  set already <-----\n");
		status = -EBUSY;
...
#if 0 
this function should be in gpio-ac83xx.c
...
int __gpio_to_irq(unsigned gpio)
{
	...
	return chip->to_irq ? chip->to_irq(chip, gpio) : -ENXIO;
}
