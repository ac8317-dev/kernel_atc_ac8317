...
static unsigned int _ac83xx_get_uartx_base_addr(int nport);
static void _ac83xx_uart_stop_tx(struct uart_port *port);

/*
 * HW functions of ac83xx uart
 */
...
static const struct of_device_id ac83xx_serial_dt_ids[] = 
{	
...
MODULE_DEVICE_TABLE(of, ac83xx_serial_dt_ids);
...
#if 0
static void _ac83xx_u0_set_trans_mode_on(void)
{
...
/*********************************************************************
// Function : void _ac83xx_uart_hwinit(void)
...
static void _ac83xx_uart_hwinit(void)
{
    ...
    */
    ...
    UART_WRITE32(0x2207E2, reg_base_addr + UART0_COMMCTRL);
   
   printk(KERN_INFO "[KERNEL UART]uart0 baud = %d,clk = %x\r\n",UART_READ32(UART0_COMMCTRL),CKGEN_READ32(0xc));  
    /* disable interrupt */
    ...
    for(nport = 1 ; nport < UART_NR ; nport ++ )
    {
        if(nport == 4)
            continue;
		reg_base_addr = _ac83xx_get_uartx_base_addr(nport) ;
#if AC83XX_UART_DEBUG	   
	...
	//itemp = UART_PINMUX_REG() & (~(UART1_PINMUX_EN | UART2_PINMUX_EN |UART3_PINMUX_EN|UART4_PINMUX_EN |UART5_PINMUX_EN));//marked by LHF
...
uint32_t _ac83xx_u0_ReadSwFifo (uint8_t u1Port, uint8_t *szBuf, uint32_t u4Len)
{
...
static uint32_t _ac83xx_u0_WriteSwFifo (uint8_t u1Port, const uint8_t *szBuf, uint32_t u4Len)
{
...
unsigned int _ac83xx_get_uartx_base_addr(int nport)
{
    if (nport < UART_NR)
...
unsigned int _ac83xx_uartx_read_allow(int nport)
{
    int ret_val;
    int data_size ;
	int reg_base_addr;
...
unsigned int _ac83xx_uartx_write_allow(int nport)
{
    int ret_val;
    int data_size ;
	int reg_base_addr;
...
unsigned int _ac83xx_uart_read_char(int nport)
{   
   while(!_ac83xx_uart_read_allow(nport)) ;
    return _ac83xx_uart_read_byte(nport);
}

void _ac83xx_uart_write_char(int nport, unsigned int byte)
{
  
  while(!_ac83xx_uart_write_allow(nport)) ;
   _ac83xx_uart_write_byte(nport,byte);
}
*/
...
void _ac83xx_uartx_get_top_err(int nport,int *p_parity, int *p_end, int *p_break)
{
	...
	reg_addr = _ac83xx_get_uartx_base_addr(nport) + UART_STATUS ;
...
static uint32_t _ac83xx_uart_write(uint8_t u1Port, const uint8_t* pData, uint32_t u4NumberToWrite)
{
    uint32_t  u4Ret = 0;
	...
	u4Ret = _ac83xx_u0_WriteSwFifo( u1Port, pData, u4NumberToWrite);
	
#if 0
    if (!TXFIFO_EMPTY (u1Port))
    {
        _ac83xx_u0_empty_int_enable(true);
    }
#endif
    return u4Ret;
}
static void _ac83xx_uart0_rx_chars(struct ac83xx_uart_port *mport)
{
	...
	//ac83xx_mask_ack_bim_irq(port->irq);
...
//				sys_kill(0, SIGUSR1);
	 ...
	 //printk(KERN_ERR "000000000000Serial: tty->buf.size %d,,tty->buf.used %d\n", tty->buf.size,tty->buf.used);
...
static void _ac83xx_uart0_tx_chars(struct ac83xx_uart_port *mport)
{
	...
	reg_base_addr = _ac83xx_get_uartx_base_addr(mport->nport);
...
static void _ac83xx_uart_tx_chars(struct ac83xx_uart_port *mport)
{
    ...
    //if(mport->nport==1)
    	//count = (UART_READ32(reg_base_addr+UART_STATUS)&0x1FFC000)>>14;//port->fifosize - 1;
//	else
	//	count = (UART_READ32(reg_base_addr+UART_STATUS)&0x1FF000)>>12;//port->fifosize - 1;

		//printk(KERN_INFO "[33333333UART Linglinglingling]int  tx chars %d.\r\n",count);
		//printk(KERN_INFO "[55555555UART Linglinglingling]int  tx chars %d.\r\n",port->fifosize - 1);
...
//static void _ac83xx_uart_work_func(struct work_struct *work)
static void _ac83xx_uart_work_func(unsigned long arg)
{
...
/*
		if (g_UartAsyncMode) 
		{
			...
			{
			  //_ac83xx_u0_empty_int_enable(false);
			  //_ac83xx_uart_stop_tx(port);
			}
		...
		//printk(KERN_INFO "[22222222UART Linglinglingling]int	tx chars.\r\n");
	...
	/* modem mode */
	//_ac83xx_uart_modem_state(mport);
...
static irqreturn_t _ac83xx_uart0_interrupt(int irq, void *dev_id)
{
	...
	if(uart_int_ident & 0x20){
		...
		UART_WRITE32(0x20,reg_base_addr+UART0_INT_STATUS);
...
static irqreturn_t _ac83xx_uart_interrupt(int irq, void *dev_id)
{
    struct ac83xx_uart_port *mport = (struct ac83xx_uart_port *)dev_id;
	unsigned int uart_int_ident;
	int reg_base_addr;
...
/*#if UART_DSP_DEBUG
	...
	if(mport->nport != 0){
		//queue_work(uart_workqueue, &mport->work);
		tasklet_hi_schedule(&mport->uart_tasklet);
	}
	else{
		...
		if (mport->fn_read_allow(mport->nport))
			...
			if (mport->fn_write_allow(mport->nport))
			{
				...
				//printk(KERN_INFO "[Lingling]int	tx chars.\r\n");
	...
	if(mport->nport != 0)
	{
	   UART_WRITE32(0x1f,reg_base_addr + UART_INT_STATUS);
	}
	else
	{
		/* Clear Local UART interrupt */
		UART_WRITE32(0xA0,reg_base_addr+UART0_INT_STATUS);
	}
...
static void _ac83xx_uart_start_tx(struct uart_port *port)
{
    ...
    if (g_UartAsyncMode) //USE ASYNC MODE  for uart 0 only
    {
        ...
        if (port->x_char)
        {
            _ac83xx_uartx_write_byte(UART_PORT0, port->x_char);
            port->icount.tx++;
            port->x_char = 0;
            return;
        }
        ...
        do
        {
            _ac83xx_uartx_write_byte(UART_PORT0, xmit->buf[xmit->tail]);
            xmit->tail = (xmit->tail + 1) & (UART_XMIT_SIZE - 1);
            port->icount.tx++;
            if (uart_circ_empty(xmit))
            {
        ...
        /*
		_ac83xx_uart_write(UART_PORT0, (const uint8_t *)(&(xmit->buf[xmit->tail])), port->fifosize);
...
#if 0  //no used
static void _ac83xx_uart_start_tx_for_printk(struct uart_port *port)
{
    //struct circ_buf *xmit = &port->info->xmit;
    struct ac83xx_uart_port *mport = (struct ac83xx_uart_port *)port;
    int count;
...
/*
static void _ac83xx_uart_start_rx(struct uart_port *port)
{
    struct ac83xx_uart_port *mport = (struct ac83xx_uart_port *)port;
    mport->rx_stop = 0;
}
...
static int _ac83xx_uart_startup(struct uart_port *port)
{
    ...
    uint32_t u4Value;
	if(mport->nport != 0)
		tasklet_init(&mport->uart_tasklet, _ac83xx_uart_work_func, (ulong)mport);

    if(mport->nport == 4)
    {
		reg_base_addr = _ac83xx_get_uartx_base_addr(mport->nport) ;
#if AC83XX_UART_DEBUG	   
		printk(KERN_INFO "[KERNEL UART]hardware init ,nport = %d ,uart base = %x\r\n",mport->nport,reg_base_addr);	  
#endif
      /* disable interrupt */
		UART_WRITE32(0, reg_base_addr + UART_INT_EN);
	   /* set baud rate  to default value */
   ...
   // printk(KERN_INFO "[Kernel uart] ac83xxuart startup _irq_allocated =%d ,mport->nport = %d ,port->irq = %d\r\n",_irq_allocated[mport->nport],mport->nport,port->irq);
    ...
    if (_irq_allocated[mport->nport] == 1)  
    {
    	if(mport->nport!=0)
    	{
	        ...
	        retval = request_irq(port->irq, _ac83xx_uart_interrupt, IRQF_SHARED,
	            "AC83xx Serial", port);
	        if (retval)
	        {
	            _irq_allocated[mport->nport] --;
    		...
    		retval = request_irq(VECTOR_RS232_1, _ac83xx_uart0_interrupt, IRQF_SHARED,
	            "AC83xx Serial", port);
	        if (retval)
	        {
	            _irq_allocated[mport->nport] --;
	...
	reg_base_addr = _ac83xx_get_uartx_base_addr(mport->nport) ;
    if(mport->nport == 1)
       UART_WRITE32(0x100FFF, reg_base_addr + UART_BUFCTRL );//For BT UART CNB00162718, MTK68527
	else
	   UART_WRITE32(0x100FFF, reg_base_addr + UART_BUFCTRL );
    ...
    struct ktermios *termios, struct ktermios *old)
{
    ...
    if(mport->nport == 0)
	    ...
	    uCOMMCTRL = _ac83xx_get_uartx_base_addr(mport->nport) + UART_COMMCTRL ;
...
static int _ac83xx_uart_request_port(struct uart_port *port)
{
    ...
    pv_region =
        request_mem_region(port->mapbase, AC83XX_UART_SIZE,
            "AC83XX Uart IO Mem");
    ...
    unsigned int count)
{
    ...
    {
        _ac83xx_uart_write(UART_PORT0, s, count);
        ...
        else
        {
            ...
            for (i = 0; i < count; i++)
            {
                while (!mport->fn_write_allow(mport->nport))
                {
                    ...
                    if(g_UartAsyncMode) //USE ASYNC MODE
                    {
                        _ac83xx_uart_write_byte(UART_PORT0, s[i]);
                    }
                    else
#endif
                    {
                ...
                if (s[i] == '\n')
                {
                    ...
                    if(g_UartAsyncMode) //USE ASYNC MODE
                    {
                        _ac83xx_uart_write_byte(UART_PORT0, "\r");
                    }
                    else
#endif
                    {
...
static int __init _ac83xx_uart_console_setup(struct console *co, char *options)
{
    ...
    printk(KERN_INFO "ac83xx console setup : uart_set_option port(%d) "
...
static int _ac83xx_uart_probe(struct device *_dev)
{
	...
	_ac83xx_uart_ports[dev->id].port.irq = irq_of_parse_and_map(np, dev->id);
	if (_ac83xx_uart_ports[dev->id].port.irq <= 0) {
		pr_err("Failed to map uart%d IRQ\n",dev->id);
		BUG();
	}
    ...
    _ac83xx_uart_ports[dev->id].port.dev = _dev;
	
    uart_add_one_port(&_ac83xx_uart_reg, &_ac83xx_uart_ports[dev->id].port);
    dev_set_drvdata(_dev, &_ac83xx_uart_ports[dev->id]);
    return 0;
}

static int _ac83xx_uart_remove(struct device *_dev)
{
    struct ac83xx_uart_port *mport = dev_get_drvdata(_dev);

    dev_set_drvdata(_dev, NULL);

    if (mport)
    {
        uart_remove_one_port(&_ac83xx_uart_reg, &mport->port);
    }

    return 0;
}

static int _ac83xx_uart_suspend(struct device * _dev, pm_message_t state)
{
#if 0
    struct ac83xx_uart_port *mport = dev_get_drvdata(_dev);
    ...
    {
        uart_suspend_port(&_ac83xx_uart_reg, &mport->port);
    }
#endif
    return 0;
}

static int _ac83xx_uart_resume(struct device *_dev)
{
#if 0
    struct ac83xx_uart_port *mport = dev_get_drvdata(_dev);
    ...
    {
        uart_resume_port(&_ac83xx_uart_reg, &mport->port);
    }
#endif
    return 0;
}

static struct device_driver _ac83xx_uart_driver =
{
    .name           = "AC83XX uart",
    ...
    .of_match_table	= of_match_ptr(ac83xx_serial_dt_ids),
};
...
//    printascii("_ac83xx_uart_init \r\n");

    /* reset hardware */
    _ac83xx_uart_hwinit();  /* all port 115200, no int */

    printk(KERN_INFO "Serial: AC83XX driver $Revision$\n");
...
#if AC83XX_UART_DEBUG		
		printk(KERN_INFO "Serial : add uart port %d , line =%d  ret = %x\n", i,ret,_ac83xx_uart_ports[i].port.line);
#endif
    }
	/*
	uart_workqueue = create_workqueue("uart_workqueue");        //create a work queue and worker thread
	 if (!uart_workqueue)
	 {
		 printk(KERN_ERR "creat uart workqueue failed\n");
		 return -ENOMEM;
	 }
	 */

    ret = driver_register(&_ac83xx_uart_driver);
    if (ret == 0)
    {
        goto out;
    }
...
static void __exit _ac83xx_uart_exit(void)
{
	...
	/*
    if (uart_workqueue)
        destroy_workqueue(uart_workqueue);        //release our work queue
        */
}
...
MODULE_DESCRIPTION("AC83XX serial port driver $Revision$");
MODULE_LICENSE("GPL");
