...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	mmc = mmc_alloc_host(sizeof(struct msmsdcc_host), &pdev->dev);  //  分配 mmc_host 结构体  同时嵌入 msmsdcc_host 结构体 这样方便以后两个结构体可以找到对方  
	if (!mmc) {                                                     //   mmc_host 结构体 是整个 SD 卡 驱动的核心   
		ret = -ENOMEM;
		goto out;
	}
	 ...
	 */
	mmc->ops = &msmsdcc_ops;                //   设置  mmc host 的操作函数  
	...
	if (stat_irqres && !(stat_irqres->flags & IORESOURCE_DISABLED)) {
		...
		ret = request_irq(host->stat_irq,                              //  申请中断   
	...
	mmc_add_host(mmc);                          //   向系统添加  mmc_host 设备  
...
{
	return platform_driver_register(&msmsdcc_driver);    //  platform driver 与 platform device 相匹配之后 调用  probe  函数   
}
