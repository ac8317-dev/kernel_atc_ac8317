...
以sdhci类型的host为例，处理完一个mmc_request之后，会执行sdhci_tasklet_finish，
而在sdhci_tasklet_finish中会调用mmc_request_done来通知host某个mmc_request已经处理完成了
***************/
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (err && cmd->retries && !mmc_card_removed(host->card)) {  // command执行出错，如果还需要重复尝试的话，这里不释放clock，只是通知mmc core
		/*
		...
		mmc_should_fail_request(host, mrq);   // 用于模拟data传输概率出错的情况
		...
		if (mrq->done)
			mrq->done(mrq);  // 执行mmc_request的回调函数来通知mmc core，对于__mmc_start_req发起的request来说，就是mmc_wait_done，上面已经说明过了
...
mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	/* 获取时钟 */
	mmc_host_clk_hold(host);    //  该函数是通过 CONFIG_MMC_CLKGATE 宏来进行使能的  
	led_trigger_event(host->led, LED_FULL);
	host->ops->request(host, mrq);  //    mmc_start_req   调用控制器驱动提供的 request 方法  发送 request     其中 host 表示 mmc 控制器   mrq  表示 request 请求  
}

static void mmc_wait_done(struct mmc_request *mrq)
{
	complete(&mrq->completion);  //  使用 complete 的目的是为了等待 request  发送完成 
}
...
static int __mmc_start_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	init_completion(&mrq->completion);  //  初始化 completion , 并设置 done 方法        // 初始化完成量，在mmc_wait_for_req_done中会去等待这个完成量
	mrq->done = mmc_wait_done; // 设置mmc_request处理完成的回调函数，会调用complete(&mrq->completion);来设置完成量
                               // host controller会调用mmc_request_done来执行这个回调函数，具体在后面分析
	if (mmc_card_removed(host->card)) {  //  如果 mmc 已经被拔出  则设置错误并返回错误    // 检测card是否存在
		mrq->cmd->error = -ENOMEDIUM;
		complete(&mrq->completion);
		return -ENOMEDIUM;
	}
	/* 调用mmc_start_request发起mmc请求 */
	mmc_start_request(host, mrq);  //  发送命令    // 开始处理mmc_request请求
	return 0;
}
				  ...
				  struct mmc_request *mrq)
{
	...
	while (1) {
		if(!wait_for_completion_timeout(&mrq->completion, DAT_TIMEOUT)){   //  等待 mmc 控制器完成 request  控制器的驱动会在完成 request 的发送后 调用 
		                                                                   //  mrq->done 方法来唤醒处于等待中的 wait_for_completion_timeout() 函数 
			printk(KERN_ERR "MSDC wait request timeout CMD<%d> ARG<0x%x>\n",mrq->cmd->opcode,mrq->cmd->arg);
			if(mrq->data && host->ops->dma_error_reset){
    ...
    if ((mrq->cmd->arg == 0) && (mrq->data) && 
        ...
        if (!cmd->error || !cmd->retries ||
                mmc_card_removed(host->card))   //  随后会检查 sd 卡是否已经被拔出   如果卡被拔出 则没有必要发送 request 可以直接调用 complete() 告知相关的等待 函数 
             break;                             //  并设置 error值  然后返回错误 

        pr_debug("%s: req failed (CMD%u): %d, retrying...\n",
                mmc_hostname(host), cmd->opcode, cmd->error);
        cmd->retries--;
        cmd->error = 0;
        host->ops->request(host, mrq);     //  没有成功  则尝试再次发送  
    }
}
 ...
 *	@areq: async request to start       // 表示新的异步请求
...
********************/
struct mmc_async_req *mmc_start_req(struct mmc_host *host,  // 目的其实是要处理本次areq   
				    struct mmc_async_req *areq, int *error)
{
    ...
    if (areq){                     ///* 为新的异步请求做准备处理 */
        ...
        mmc_pre_req(host, areq->mrq, !host->areq);   //  所以一开始对areq做 预准备
	...
	/*
	一个很明显的等待操作，从函数名就可以得知这是一个同步等待，会放弃处理器等待命令完成。但从上文可以看到，
	一直还未正在处理命令，也没往host发送命令，此时就开始等待命令完成显然是毫无道理。但有时候代码容易看漏，
	该等待是针对的host->areq，并不是参数传递的areq。本次等待的是上一次传递未完成的动作。如果上次的传输以及完成，
	则该等待函数会很快返回。并把成功还是错误的情况反馈给外面的mmc_blk_issue_rw_rq()

	对上一次发起的、正在处理、等待完成的异步请求进行处理、等待操作 */
	if (host->areq) {
		mmc_wait_for_req_done(host, host->areq->mrq);    // 在这里等待正在处理的异步请求处理完成
		host->ops->send_stop(host, host->areq->mrq);     //add for MTK msdc host <Yuchi Xu>
		do{  //   以下是错误处理部分  
			 // if card was removed, do not tuning it. Aync request always has data
			if ((host->areq->mrq->cmd->error == -ENOMEDIUM) || (host->areq->mrq->data->error == -ENOMEDIUM))
				break;
			
			host->ops->req_tuning(host, host->areq->mrq);	//add for MTK msdc host <Yuchi Xu>
			retry_times++;
	...
	/* 对新的异步请求进行发起操作   真正本次的处理如果不是urgent事件的话  */
	if (!err && areq) {   
...
#endif
//  和之前的mmc_pre_req(host, areq->mrq, !host->areq); 对应起来。试想一下，如果想在命令前或命令后做自定义的事情，则可以考虑在这里添加。  
...
// 如果这其中没发生错误，host->areq = areq; 就保存起来了，即current操作变成pre操作。
//并且这里面不需要在做等待，因为等待的操作将在下次函数在进来时的第2步进行。
//可以看出设计者为了最大化数据吞吐量，把函数设计成最大限度的流水线处理，压缩所有可能的耗时操作。
//试想如果不这么做，则每次操作都需要完成准备，等待，准备，等待的循环。函数设计成这样，
//则把同步等待的时间利用起来，做另一次传输的准备，减少无谓的带宽损失。
	if (err)
		host->areq = NULL;
	else
		host->areq = areq;
...
//把参数state赋值为函数返回值err，返回上一次的传输结果，host->areq ，为什么？因为本次的传输肯定还未完成，
//需要等待硬件处理，但上次的host->areq已经完成，可以处理后续事情。这就是为什么mmc_blk_issue_rw_rq()在发起命令后
//返回需要mq_rq = container_of(areq, struct mmc_queue_req, mmc_active);用这样的方式获得 mmc_queue_req。
	if (error)
		*error = err;
	return data;          // 反回上一次正常处理的异步请求
}
...
//  mmc 是如何将数据交给 mmc/sdcard 的 控制器 并让后者将数据发送出去的呢   就是通过以下的函数  
void mmc_wait_for_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	__mmc_start_req(host, mrq);  // 发送命令    // 开始发起mmc_request请求
	mmc_wait_for_req_done(host, mrq);    //  等待命令完成  
}
...
int mmc_wait_for_cmd(struct mmc_host *host, struct mmc_command *cmd, int retries)
{
	...
	memset(cmd->resp, 0, sizeof(cmd->resp));   //  清空应答    // 清空command的response
	cmd->retries = retries;                    // 失败时的重复尝试次数

	mrq.cmd = cmd;                             // 封装到mmc_request中
	cmd->data = NULL;   //   保存命令             // 不带数据包的命令，故清空data

	mmc_wait_for_req(host, &mrq);  //  发送命令  并等待 response     // 调用mmc_wait_for_req发起mmc请求并且等待其处理完成
	...
	return cmd->error;               // 返回错误码
		...
		/*********
		所有需要发送的CMD都由mmc_command进行封装，在函数内部被mmc_request 
 ...
 */
	...
	/* 
	驱动中使用mmc_claim_host(host);来得知,当前mmc控制器是否被占用,当前mmc控制器如果被占用,
	那么 host->claimed = 1;否则为0,如果为1,那么会在for(;;)循环中调用schedule切换出自己,当占用mmc控制器
	的操作完成之后,执行 mmc_release_host()的时候,会激活登记到等待队列&host->wq中的其他程序获得mmc主控制器的物理使用权
	*/
...
//这个函数和mmc_claim_host(host);配对使用，相当于一把锁，就是在同一个时间只有一个sd卡可以保持和主控制器通讯；
...
static inline void mmc_set_ios(struct mmc_host *host)
{
	...
	host->ops->set_ios(host, ios);   //  调用  host 中  msm_sdcc.c 中的 sdcc_ops 函数进行寄存器的 set_ios 设置寄存器  
}                                    // 调用host->ops->set_ios来对mmc总线的io setting进行设置，核心函数
...
u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
	...
	if (bit) {
		...
		pr_warning("%s: host doesn't support card's voltages: ocr= %d\n",
...
static void mmc_power_up(struct mmc_host *host)
{
	...
	if (mmc_host_is_spi(host))
		...
		host->ios.chip_select = MMC_CS_DONTCARE;  // 设置总线模式
...
void mmc_power_off(struct mmc_host *host)
{
	...
	/*
	 * For eMMC with MMC_CAP2_NO_SLEEP_CMD, call mmc_poweroff_notify()
	 * at shutdown flow.
	 */
	else if (host->card && mmc_card_mmc(host->card)
...
void mmc_detect_change(struct mmc_host *host, unsigned long delay)
{
	...
	spin_unlock_irqrestore(&host->lock, flags);
#endif
	host->detect_change = 1;    //  设置 mmc_host -> detect_change , 表示检测到  card 的状态发生了变化   

	wake_lock(&host->detect_wake_lock);
	ret = mmc_schedule_delayed_work(&host->detect, delay); // 间隔 delay 时间之后执行工作 host_detect     这个 delay 用于消斗  
	                                                      //  这里调用  host 的工作队列  而在这里的  delay 就是  mmc_rescan 函数     
...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
	 ...
	 */
	if (!(host->caps2 & MMC_CAP2_FUNC_EMMC)) {
		sdio_reset(host);
	}

	// Make card to idle state
	mmc_go_idle(host);

	if (!(host->caps2 & MMC_CAP2_FUNC_EMMC)) {
		mmc_send_if_cond(host, host->ocr_avail);    //  发送 sd2.0 协议支持的命令   设备初始化命令      // 获取card的可用频率，存储到host->ocr_avail中
	}
	
	if (host->caps2 & MMC_CAP2_FUNC_EMMC){
		printk(KERN_ERR "----------------->>> [msdc%d] MMC_CAP2_FUNC_EMMC <<<------------------\r\n", host->index);
		if (!mmc_attach_mmc(host))
			return 0;
	}
	else{
		/* Order's important: probe SDIO, then SD, then MMC */
		if (!mmc_attach_sdio(host))   //  这里会判断是  sdio  sd  mmc  三种设备的哪一种    因为三种设备的协议有一些差别  
			return 0;
		if (!mmc_attach_sd(host))     //  所以要先判断出是哪一种设备后才好调用相应的操作函数  
			return 0;
		if (!mmc_attach_mmc(host))     // 以 mmc_attach_sd 为例  
			return 0;
	}
...
{
	static const unsigned freqs[] = { 400000, 300000, 260000, 200000 };
	struct mmc_host *host =
		container_of(work, struct mmc_host, detect.work);//container_of()这个函数是由内核黑客为了我们能从一个结构体的某个成员的指针来得到整个结构体的指针所写的一个函数 
	int i;
	int init_ret = 0;
	bool extend_wakelock = false;

	if (host->rescan_disable)  //   如果host 还没有初始化完成的话  会设置 rescan_disable 此时是不允许扫描硬件总线的      
		return;

	if (host->ops->get_rescan)
	{
		host->ops->get_rescan(host);
		printk("[MSDC%d] ~~~~ get_rescan  ~~~~\n", host->index);  //  xl 4
	}
	 ...
	 * still present
	 */
	 //  这里是先处理总线上原来已经存在 card 的情况 
	if (host->bus_ops && host->bus_ops->detect && !host->bus_dead
	    && !(host->caps & MMC_CAP_NONREMOVABLE))  // 这里所做的事情:  当我们进入 delay work 时   我们首先要检查一下,目标卡是否依然存在，如果仍然存在而且目标卡是可插拔的，
		host->bus_ops->detect(host); // 那么我们需要先将该卡在内核留下的一些痕迹清除，重新探测该卡。所以一般如果是目标卡复位的话，一般是调用内核提供的专用的复位接口(如sdio_reset_comm等)，而不是调度host的这个delay work
	 ...
	 */
	mmc_bus_put(host); // 因为在这个函数的前面已经获取了一次host，可能导致host->bus_ops->detect中检测到card拔出之后，没有真正释放到host的bus，所以这里先put一次
	                   // host bus的计数（bus_refs）为0的时候，会调用__mmc_release_bus清空host bus的信息
	mmc_bus_get(host);  // 再获取host bus

 
	//判断是否bus的引用计数是否为0,如果不为0 ，说明bus仍然被引用，不能再继续下面的探测了。
	...
	if (host->ops->get_cd && host->ops->get_cd(host) == 0)   //  调用 get_cd 的方法   看看是不是有卡在里面  
	{
		if(host->enabledelay)  // 说明当前有 card 插入  调用 mmc_schedule_delayed_work 以500 ms 的频率 来识别和初始化 card  
		{
			host->enabledelay = 0;
			mmc_schedule_delayed_work(&host->detect, msecs_to_jiffies(500));
		}
		goto out;
	}
	...
	*/
	...
	for (i = 0; i < ARRAY_SIZE(freqs); i++) {
		if (init_ret = !mmc_rescan_try_freq(host, max(freqs[i], host->f_min))) {  // 对card 类型探测和进行初始化    以一定频率搜索host bus上的card
	...
	if (host->ops->init_card_status){
		if (init_ret)
			host->ops->init_card_status(host, MMC_INIT_CARD_STATUS_SUCCESS);
		else
			host->ops->init_card_status(host, MMC_INIT_CARD_STATUS_FAILED);
	}
	mmc_release_host(host);   //这个函数和mmc_claim_host(host);配对使用，相当于一把锁，就是在同一个时间只有一个sd卡可以保持和主控制器通讯；
...
// 当底层host controller调用mmc_add_host来注册host时，在mmc_add_host中就会调用mmc_start_host来启动一个host
// 相对应的，会在mmc_remove_host中调用mmc_stop_host停止host
void mmc_start_host(struct mmc_host *host)   //  启动一个 host 的时候   会去扫描  mmc 硬件总线   
{
	host->rescan_disable = 0;
	mmc_power_off(host);
	/* 到这里host已经可以工作了，可以开始进行后续的card操作了 */
	mmc_detect_change(host, 0);   //  当启动一个 host 的时候  并不知道 当前是否有 card 插入   所以需要假设卡的状态发生了变化  并且进行第一次mmc 硬件的扫描  
...
int mmc_resume_host(struct mmc_host *host)
{
	...
	if (host->bus_ops && !host->bus_dead) {
		...
		BUG_ON(!host->bus_ops->resume);
		mmc_delay(100);//add by dandan liu
		err = host->bus_ops->resume(host);
		if (err) {
			// Max Xia Add Retry Here
			...
			{
				pr_warning("------------------->>>>> %s: card resume fail and retry successfully <<<<<-------------------\n", mmc_hostname(host));
			}
		}
	}
...
static int __init mmc_init(void)
{
	...
	workqueue = alloc_ordered_workqueue("kmmcd", 0);  //  创建一个工作队列 这个工作队列将用于扫描sd卡设备
	if (!workqueue)
		return -ENOMEM;

	ret = mmc_register_bus();    //  注册 mmc 总线   总线提供 probe 方法    并直接在内部调用 驱动的 probe 方法      // 会生成/sys/bus/mmc目录
	if (ret)
		goto destroy_workqueue;  

	ret = mmc_register_host_class();  //  注册 名为 mmc_host的类                // 会生成/sys/class/mmc_host目录
	if (ret)
		goto unregister_bus;

	ret = sdio_register_bus();   //  注册 sdio 总线   总线提供 probe 方法    并直接在内部调用 驱动的 probe 方法  
	if (ret)
