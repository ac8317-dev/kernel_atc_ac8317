...
int mmc_app_cmd(struct mmc_host *host, struct mmc_card *card)
{
	...
	err = mmc_wait_for_cmd(host, &cmd, 0);   //  发送 CMD 并等待 (阻塞方式)
	if (err)
		return err;

	/* Check that card supported application commands */
	/* 检查card status 第五位 判断sd卡是否支持 ACMD  */
	...
	struct mmc_command *cmd, int retries)
{
	 ...
	 */
	for (i = 0;i <= retries;i++) {
		err = mmc_app_cmd(host, card);   //  发送 CMD55 
		if (err) {
		...
		mmc_wait_for_req(host, &mrq);   //  发送 ACMDx  

		err = cmd->error;
		if (!cmd->error)
			break;               //  发送成功 则直接 break 并返回  
