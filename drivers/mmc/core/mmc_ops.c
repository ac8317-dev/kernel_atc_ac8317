...
#define CARD_IDLE_RETRY	6
	for (i=1; i<CARD_IDLE_RETRY; i++)
	{
		/*
		...
		cmd.opcode = MMC_GO_IDLE_STATE;   //  发送 cmd0  使 sd 卡处于 空闲状态  
		cmd.arg = 0;
		...
		//printk("cmd.resp[0] = 0x%x\n", cmd.resp[0]);
		if (((cmd.resp[0] >> 9) & 0xf) == 0) {
			break;
		}

		mmc_delay(10*i);
	}
	if (CARD_IDLE_RETRY == i)
		printk("@@@@@@ reset card faile @@@@@\n");
...
在idle状态时，向卡传送Host支持的电压范围，卡回复OCR的值以及上电复位的状态。如果发送的电压参数为0，则卡仅传回OCR的值，并不进行判断。
...
int mmc_all_send_cid(struct mmc_host *host, u32 *cid)
{
	...
	err = mmc_wait_for_cmd(host, &cmd, MMC_CMD_RETRIES);  //  发起并等待请求完成  
	if (err)
		return err;
...
/*****************************************
int mmc_send_power_csd(struct mmc_card *card, u8 *ext_csd)    //  xl  
{
	return mmc_send_cxd_data(card, card->host, MMC_GEN_CMD,
			ext_csd, 512);
}
...
特殊之处在于：在__mmc_switch中会发起CMD6命令，
会导致card进入programming state，因此，在__mmc_switch中必须去获取card的status，直到card退出programming state。这部分就是通过CMD13来实现的
*************/
	       ...
	       unsigned int timeout_ms)
{
	...
	cmd.opcode = MMC_SWITCH;   /* 主要是根据对应命令构造struct mmc_command */
	...
	do {
		err = mmc_send_status(card, &status);/* 调用mmc_send_status发送CMD13获取card status，等待card退出programming state。 */
		if (err)
			return err;
...
int mmc_send_status(struct mmc_card *card, u32 *status)
{
	 ...
	 */
	if (status)  /* 对response的处理 */
		*status = cmd.resp[0];   // 依照协议，response[0]存储了status，因此这里提取cmd.resp[0]

	return 0;
}
/*******
注: mmc_go_idle、mmc_select_card、mmc_all_send_cid、mmc_set_relative_addr、mmc_send_cxd_native等等
的实现方法和其类似。
主要差异在于命令的构造区别以及对response的数据的处理
***********/
