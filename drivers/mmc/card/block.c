...
/*
 * boot_pinfo save boot partiton info, 
 * shart to add_partition function in partition-generic file
 */
struct boot_partition_info boot_pinfo[32];
...
static DECLARE_BITMAP(name_use, 256);
	...
	#define MMC_SUPPORT_BOOT_MULTI_PART 1
	#define BOOT_FROM_SD_SLOT			(0UL)
#elif defined (CONFIG_BOOT_SD2)
	#define MMC_SUPPORT_BOOT_MULTI_PART 1
	#define BOOT_FROM_SD_SLOT			(2UL)
#else
	#define MMC_SUPPORT_BOOT_MULTI_PART 0
#endif

#if MMC_SUPPORT_BOOT_MULTI_PART
	#define BOOT_SDMMC_MAX_PARTITIONS   8
#endif
...
/* identify the existence of eMMC expdb partiton on board */
int emmcpart_expdb_exist = 0;
	...
	struct mmc_blk_request *brq, int *ecc_err)
{
	...
	for (retry = 2; retry >= 0; retry--) {
		...
		// When card was removed, do not try
		if (err == -ENOMEDIUM)
			break;
...
// 整个函数的宗旨就是填充各种结构体，用正确的值，譬如cmd号，是读还是写，是单块还是多块  也包含一些特殊情况需要做的事情，譬如特殊命令
			       ...
			       struct mmc_queue *mq)
{
	...
	struct mmc_blk_request *brq = &mqrq->brq;   //  将来这个mrq将是承载命令发送的结构  
	...
	//	另一个该函数重要的工作，是mmc_queue_map_sg。要把request里面包含的数据buffer指针，给map到data.sg结构里面  
	brq->data.sg = mqrq->sg;
	brq->data.sg_len = mmc_queue_map_sg(mq, mqrq);

	/*
	
	 *sg; 结构是分散聚拢DMA的描述，从这点可以看出mmc host的处理是通过dma来完成，sgdma的好处是，
	 它可以处理非连续的多个命令，而不需要cpu干扰。Cpu只需要填充好命令，剩下的事情交个dma处理即可。
	 简单说就是，普通dma可以处理单个命令，sgdma可以在一次dma里面处理一组命令
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	struct request *req;           //  Req参数变换名称struct request *rqc   
	struct mmc_async_req *areq;
	...
	if (!rqc && !mq->mqrq_prev->req)
		return 0;
	time1 = sched_clock();
#if defined (STORAGE_TRACE_ENABLE)	
	if( rqc )
	{
	...
	do {
		if (rqc) {
			mmc_blk_rw_rq_prep(mq->mqrq_cur, card, 0, mq);  //如果req有值，则进入这个关键的函数  做一些准备工作  取到异步request结构体 areq    
			areq = &mq->mqrq_cur->mmc_active;
		} else
			areq = NULL;
		areq = mmc_start_req(card->host, areq, (int *) &status);  //  正式启动  传输  
		if (!areq)
			return 0;
		...
		switch (status) {        //  通过switch case来判断status返回的是什么状态  
		...
		case MMC_BLK_PARTIAL:
			...
			mmc_blk_reset_success(md, type);
			spin_lock_irq(&md->lock);
			ret = __blk_end_request(req, 0,           //   完成本次传输，说明数据已正确读写  
...
//				met_mmc_end(card->host, card->host->areq);
    ...
    if ((1 == g_mtk_mmc_perf_dbg) && (2 == g_mtk_mmc_dbg_range)){
		...
		case MMC_BLK_CMD_ERR:
			...
			if (!mmc_blk_reset(md, card->host, type))               //  把控制器都reset一遍   要做重新上下电的动作
				break;
			goto cmd_abort;
		case MMC_BLK_RETRY:
			if (retry++ < 5)
		...
		case MMC_BLK_ABORT:
			if (!mmc_blk_reset(md, card->host, type))
				break;
			goto cmd_abort;
		case MMC_BLK_DATA_ERR: {     //  也要reset控制器   
			int err;
		...
		//  并且发现是多块读，则切换到单块读，如果还是失败，没办法blk_end_request(req, -EIO, 给上层直接EIO的错误  
		case MMC_BLK_ECC_ERR:
			...
			if (!ret)
				goto start_new_req;
			break;
		case MMC_BLK_NOMEDIUM:   //  如果是MMC_BLK_NOMEDIUM没设备了，直接退出  
			goto cmd_abort;
		}

		if (ret) {  //  命令完成之后，对命令的完成状态做各种判断，是否正确完成，是否出错，是否需要retry  
			/*
			 * In case of a incomplete request
			 * prepare it again and resend.
			 */
			mmc_blk_rw_rq_prep(mq_rq, card, disable_multi, mq);
			mmc_start_req(card->host, &mq_rq->mmc_active, NULL);   //  剩余的都是为了命令出错处理，或者重试  
		}
	} while (ret);
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (req && req->cmd_flags & REQ_DISCARD) {  //   根据 req->cmd_flags 的命令做不同的事情   
		/* complete ongoing async transfer before issuing discard */
		if (card->host->areq)
			mmc_blk_issue_rw_rq(mq, NULL);   //  开始读写   
					      ...
					      int area_type)
{
	...
	int devidx, ret;
		...
		{
			printk(KERN_ERR " --> [mmc_blk_alloc_req]devidx number is error. <--\n");
...
************/
	...
	md = kzalloc(sizeof(struct mmc_blk_data), GFP_KERNEL);     //  分配 mmc_blk_data 结构体 md 并初始化  同时 mmc_queue 作为 mmc_blk_data 的成员也被创建  
	if (!md) {
		ret = -ENOMEM;
		goto out;
	}
	...
	md->disk = alloc_disk(perdev_minors);  //  分配 gendisk 结构体保存到 md中 , gendisk 与磁盘设备相对应 
	if (md->disk == NULL) {
		ret = -ENOMEM;
		goto err_kfree;
	}
	...
	ret = mmc_init_queue(&md->queue, card, &md->lock, subname);   //  创建并初始化请求队列  
	if (ret)
		goto err_putdisk;
...
static int mmc_add_disk(struct mmc_blk_data *md)
{
	...
	add_disk(md->disk);   //  这个方法一旦被调用就代表磁盘被激活了  系统随时都可能调用 该磁盘提供的各中方法 
	                      //  甚至在该函数返回之前就会调用  因此在完成磁盘的初始化之前不要调用  add_disk  
...
struct mmc_part_info {
	...
	u32 sector_offset;			/* offset in sector */
	u32 sector_size;			/* size of sector */
	u32 mask_flags;				/* mask flags */
};

static struct list_head mmc_parts;		/* partitions */

//--------------------------------------------------------
static char *command_line_next_arg(char *args, char **param, char **val)
{
...
static unsigned long long partition_size_parse (const char *const ptr, const char **retptr)
{
	unsigned long long ret = simple_strtoul(ptr, (char **)retptr, 0);
...
//
// size@offset(partition_name)mask
//
static int parse_partition_info(char* parts_info, uint32_t card_size_in_blocks)
{
	char* p = parts_info;
	unsigned int i;
	int comma = -1;
	char *next = NULL;
	char part[64];
	char *size, *offset, *name, *mask;
	const char * sizeptr;
	char * partptr;
	struct mmc_part_info *new_part;
	u32 last_offset = 0;
	u32 last_size = 0;
	u8 is_userdata_partition = 0;
	struct list_head *part_ptr;
	struct mmc_part_info *entry;

	while (p && (*p != '\0'))
	{
		// for partition 'userdata'
		if(*p == '-')
		{
			is_userdata_partition = 1;
			// skip "-"
			p = p + 1;
		}
		else
		{
			is_userdata_partition = 0;
		}

		// find comma (,) , get one partition information string
		comma = -1;
		for (i = 0; p[i]; i++) 
		{
			if (p[i] == ',')
		...
		//
		// handle partition string
		//

		// copy to temp buffer
		if (comma != -1)
		{
			strncpy(part, p, comma);
			part[comma] = '\0';
		}
		else
		{
		...
		if (comma != -1)
		{
			size = partptr;
			partptr[comma] = '\0';
			partptr = partptr + (comma+1);
		}

		// find comma ( ( ), get partition offset
		comma = -1;
		for (i = 0; partptr[i]; i++) 
		{
			if (partptr[i] == '(')
		...
		// convert size to block
		sizeptr = size;
		u32 size_in_block = partition_size_parse(size, &sizeptr) / 512; // in block

		new_part = kmalloc(sizeof(struct mmc_part_info), GFP_KERNEL);
		if(!new_part)
		{
    		return -1;
		}
		memset(new_part, 0, sizeof(struct mmc_part_info));
		strcpy(new_part->name, name);
		if (is_userdata_partition)
		{
			new_part->sector_offset = last_offset + last_size;
			new_part->sector_size = card_size_in_blocks - new_part->sector_offset;
		}
		else
		{
			new_part->sector_offset = simple_strtoull(offset, NULL, 0) / 512;
			new_part->sector_size = size_in_block;
		}
		new_part->mask_flags = simple_strtoul(mask, NULL, 0);
		list_add_tail(&new_part->part_node, &mmc_parts);
		if (new_part->sector_offset > last_offset) {
			last_offset = new_part->sector_offset;
			last_size = new_part->sector_size;
		}
		
		//pr_err("-----> Parse Result: name=%s, offset=%s, size=%s(%d), mask=%s <-----", name, offset, size, size_in_block, mask);
		p = next;
	}
...
static int parse_command_line_for_partitions(char* command_line, uint32_t card_size_in_blocks)
{
	int ret = -1;
	char *arg_name, *arg_val;
	
	struct list_head *entry, *n;
	struct mmc_part_info *part_tmp;
	...
	// check list that stroe the partition information
	ret = list_empty(&mmc_parts);
	if (!ret) //not empty
	{
		// empty list and release resource
		list_for_each_safe(entry, n, &mmc_parts) 
		{
			part_tmp = list_entry(entry, struct mmc_part_info, part_node);
			list_del(entry);
			kfree(part_tmp);
		}
		INIT_LIST_HEAD(&mmc_parts);
	}
	...
	// parse 1: get the partition string from whole command line string
	while (*command_line)
	{
		command_line = command_line_next_arg(command_line, &arg_name, &arg_val);
		if (strncmp(arg_name, "parts", 5) == 0)
		{
			// parse 2: get each partition information and add to list
			ret = parse_partition_info(arg_val, card_size_in_blocks);
		}
	}
...
#endif //MMC_SUPPORT_BOOT_MULTI_PART
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	char cap_str[10];
	int ret = 0;
	uint32_t card_size_in_blocks = 0;
	...
	// parse command line that been past from uboot, and get partitions information
	char* kernel_cmd_line = saved_command_line;
	//pr_err("Kernel Command Line (in block driver): %s", kernel_cmd_line);
	...
	printk("[%s]: %s by manufacturer settings, quirks=0x%x\n", __func__, md->disk->disk_name, card->quirks);    //   xl 1   bug 

#ifdef CONFIG_MMC_BLOCK_DEFERRED_RESUME
	mmc_set_bus_resume_policy(card->host, 1);
#endif
	if (mmc_add_disk(md))    //  为了讲一个磁盘添加到系统中  对系统可用  必须初始化磁盘数据结构  并调用 add_disk 方法  
		goto out;
	...
	//if ((card->host->index == 2) || (card->host->index == 0))
	if (card->host->index == BOOT_FROM_SD_SLOT)  // only in slot 0 or slot 2
	{
		printk("---------------------------------->>> Start Parse Partitions Info <<<---------------------------------\n");
		if (mmc_card_sd(card))
		{
			printk("<1>""mmc_blk_probe sd card in slot%d,and size:%d, 0x%x bytes\r\n", card->host->index, card->csd.capacity, card->csd.capacity);
			card_size_in_blocks = card->csd.capacity;
		}
		else if (mmc_card_mmc(card))
		{
			printk("<1>""mmc_blk_probe sd card in slot%d,and size:%d blocks\r\n", card->host->index, card->ext_csd.sectors);    //  xl 2 
			card_size_in_blocks = card->ext_csd.sectors;
		}

		ret = parse_command_line_for_partitions(kernel_cmd_line, card_size_in_blocks);
		if (ret == -1)
		{
			pr_err("-----> parse partitions information from command line failed. <-----");
		}
		else
		{		
			i = 0;
			list_for_each_entry(entry, &mmc_parts, part_node)
			{
				strcpy(boot_pinfo[i].part_name, entry->name);

				/* check if expdb partition is existing */
				if (!strcmp(boot_pinfo[i].part_name, "expdb"))
					emmcpart_expdb_exist = 1;

				boot_pinfo[i].part_offset = entry->sector_offset;
				boot_pinfo[i].part_size = entry->sector_size;
				printk("----> mount partition(%d): name=%s offset= 0x%x size=%d <----\n", i, boot_pinfo[i].part_name, 
					boot_pinfo[i].part_offset, boot_pinfo[i].part_size);                                                     //  xl 3
				hd = add_partition_ext(md->disk, i, boot_pinfo[i].part_offset, boot_pinfo[i].part_size, ADDPART_FLAG_NONE, NULL);
				i++;
			}
		}
	...
	{
		/* printk("<1>""mmc_blk_probe sd card in slot%d\r\n", card->host->index); */
	}
#endif
...
static int __init mmc_blk_init(void)
{
	...
	res = mmc_register_driver(&mmc_driver);  //  向系统注册一个 mmc_driver ,在与 ac83xx-device.c  中注册的 mmc_device 相匹配之后 会调用 mmc_blk_probe() 
	if (res)                                 //  来申请一个块设备文件  最终SD卡的所有驱动 都是通过这个块设备文件来调用的   也就是说  这个块设备文件是 kernel 与上层的接口
		goto out2;
