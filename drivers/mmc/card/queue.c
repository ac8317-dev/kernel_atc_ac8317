...
static int mmc_queue_thread(void *d)
{
	...
	down(&mq->thread_sem);
	do {
		struct request *req = NULL;   //  用来提取req  
...
#endif     //  存储到这次处理mqrq_cur里面mq->mqrq_cur->req = req  
		req = blk_fetch_request(q);   //  从块设备队列提取存储的req  可以多次调用，如果queue里面没有内容，req将返回NULL。  
#ifdef MTK_IO_PERFORMANCE_DEBUG
		...
		if (req || mq->mqrq_prev->req) {
			...
			mq->issue_fn(mq, req);                //   对req进行处理    mmc_blk_issue_rq 
		} else {
			...
			up(&mq->thread_sem);
			schedule(); //  倘若req || mq->mqrq_prev->req 这次获取的req和上次的req都为NULL的话，线程进入睡眠状态kthread_should_stop() –> schedule()
			down(&mq->thread_sem);
		}

		/* Current request becomes previous request and vice versa. 处理完毕后把mq->mqrq_prev = mq->mqrq_cur， 然后清空mq->mqrq_cur  */
