...
// 定义了图形卡的硬件特性，是不能改变的
//fb_fix_screeninfo结构体中，记录了用户不能修改的固定显示控制器参数。
//这些固定的参数如缓冲区的物理地址、缓冲区的长度等等
struct fb_fix_screeninfo {
	...
	__u32 visual;			/* see FB_VISUAL_*	屏幕使用的色彩模式 	*/ 
	__u16 xpanstep;			/* zero if no hardware panning  如果没有硬件panning ，赋0  */
	__u16 ypanstep;			/* zero if no hardware panning  */
	__u16 ywrapstep;		/* zero if no hardware ywrap  1行的字节数    */
 ...
 */
struct fb_bitfield {
	__u32 offset;			/* beginning of bitfield  位域偏移	*/
...
struct fb_var_screeninfo {
	...
	__u32 bits_per_pixel;		/* guess what		每像素位数，BPP  	*/
	__u32 grayscale;		/* 0 = color, 1 = grayscale,	非0时指灰度 */
					/* >1 = FOURCC			*/
	struct fb_bitfield red;		/* bitfield in fb mem if true color, */
	struct fb_bitfield green;	/* else only length is significant */
	struct fb_bitfield blue;    //   	fb缓存的R\G\B位域   
...
// 描述设备无关的颜色映射信息。可以通过FBIOGETCMAP和FBIOPUTCMAP 对应的ioctl操作设定或获取 颜色映射信息
...
struct fb_ops {
	...
	/* set the video mode according to info->var   设置视频模式   */
	int (*fb_set_par)(struct fb_info *info);

	/* set color register  设置color寄存器的值  */
	int (*fb_setcolreg)(unsigned regno, unsigned red, unsigned green,
			    unsigned blue, unsigned transp, struct fb_info *info);

	/* set color registers in batch 批量设置color寄存器，设置颜色表   */
	...
	/* wait for blit idle, optional  等待blit空闲 */
	int (*fb_sync)(struct fb_info *info);

	/* perform fb specific ioctl (optional)  fb特定的ioctl操作  */
	int (*fb_ioctl)(struct fb_info *info, unsigned int cmd,
			unsigned long arg);

	/* Handle 32bit compat ioctl (optional) 处理32兼容的ioctl操作  */
	int (*fb_compat_ioctl)(struct fb_info *info, unsigned cmd,
			unsigned long arg);

	/* perform fb specific mmap   fb特定的mmap操作 */
	int (*fb_mmap)(struct fb_info *info, struct vm_area_struct *vma);

	/* get capability given var   通过fb_info获得framebuffer的能力  */
...
struct fb_info {
	...
	struct fb_var_screeninfo var;	/* Current var */                 //当前缓冲区的可变参数，重点
	struct fb_fix_screeninfo fix;	/* Current fix */                 //固定参数，重点
	struct fb_monspecs monspecs;	/* Current Monitor specs */       //显示器标准
	struct work_struct queue;	/* Framebuffer event queue */         //帧缓冲区队列
	...
	struct list_head modelist;      /* mode list */                   //模式链表
	struct fb_videomode *mode;	/* current mode */                    //当前video模式
	...
	struct device *device;		/* 父设备  This is the parent */
	struct device *dev;		/*   fb设备  This is this fb device */
	int class_flag;                    /* 私有的sysfs标志  private sysfs flags */
#ifdef CONFIG_FB_TILEBLITTING
	struct fb_tile_ops *tileops;    /* Tile Blitting 图块blitting */
#endif
	char __iomem *screen_base;	/* Virtual address */                       //显存基地址
	unsigned long screen_size;	/* Amount of ioremapped VRAM or 0 */        //显存大小  
	void *pseudo_palette;		/* Fake palette of 16 colors */             //伪16色颜色表
#define FBINFO_STATE_RUNNING	0
#define FBINFO_STATE_SUSPENDED	1
	u32 state;			/* Hardware state i.e suspend */                    //硬件状态，如挂起
	void *fbcon_par;                /* fbcon use-only private area */       //用作私有数据区
...
extern struct fb_info *registered_fb[FB_MAX];   /* 次设备号有没有大于规定的最大值32 */  
