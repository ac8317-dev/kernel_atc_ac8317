...
static void snd_soc_instantiate_card(struct snd_soc_card *card)
{
	...
	ret = snd_card_create(SNDRV_DEFAULT_IDX1, SNDRV_DEFAULT_STR1,       //  xl  1 
...
{
	struct snd_soc_card *card = platform_get_drvdata(pdev);   //   xl  从 platform_device 参数中取出 snd_soc_card
	int ret = 0;
	...
	ret = snd_soc_register_card(card); //  xl 通过 snd_soc_register_card，为 snd_soc_pcm_runtime 数组申请内存
	if (ret != 0) {
			   ...
			   int num_dai)
{
	...
	codec->name = fmt_single_name(dev, &codec->id);   //  xl  确定codec的名字，这个名字很重要，Machine驱动定义的snd_soc_dai_link中会指定每个link的codec和dai的名字，进行匹配绑定时就是通过和这里的名字比较，从而找到该Codec的
	if (codec->name == NULL) {
		kfree(codec);
		return -ENOMEM;
	}
	...
	/* register any DAIs */
	if (num_dai) {
		ret = snd_soc_register_dais(dev, dai_drv, num_dai);      //  xl    通过snd_soc_register_dais函数对本Codec的dai进行注册
		if (ret < 0)
			goto fail;
	}

	mutex_lock(&client_mutex);
	list_add(&codec->list, &codec_list);   //  xl  注册到全局的 code 链表中  
	snd_soc_instantiate_cards();            // 触发Machine驱动进行一次匹配绑定操作    
	mutex_unlock(&client_mutex);
