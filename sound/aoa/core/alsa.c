...
int aoa_alsa_init(char *name, struct module *mod, struct device *dev)
{
	...
	alsa_card->dev = dev;
	strlcpy(alsa_card->driver, "AppleOnbdAudio", sizeof(alsa_card->driver));   // xl  chip register id  important 
	strlcpy(alsa_card->shortname, name, sizeof(alsa_card->shortname));          //  for  printk 
	strlcpy(alsa_card->longname, name, sizeof(alsa_card->longname));            //  show proc 
		       ...
		       void * device_data, struct snd_device_ops * ops)
{
	...
	err = snd_device_new(card, type, device_data, ops);   //  xl The logical device for creating sound card   把该pcm挂到声卡中
	if (err) {                                              //  such as pcm  midi  mixer  and so on 
