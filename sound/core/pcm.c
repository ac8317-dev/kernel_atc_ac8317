		...
		struct snd_pcm **rpcm)
{
	...
	static struct snd_device_ops ops = {
		...
		.dev_register =	snd_pcm_dev_register,            //   xl 1
	...
	if ((err = snd_pcm_new_stream(pcm, SNDRV_PCM_STREAM_PLAYBACK, playback_count)) < 0) {  // xl 创建播放流
		snd_pcm_free(pcm);
		return err;
	}
	if ((err = snd_pcm_new_stream(pcm, SNDRV_PCM_STREAM_CAPTURE, capture_count)) < 0) {     // xl 创建录音流
		snd_pcm_free(pcm);
		return err;
	}
	mutex_init(&pcm->open_mutex);
	init_waitqueue_head(&pcm->open_wait);
	if ((err = snd_device_new(card, SNDRV_DEV_PCM, pcm, &ops)) < 0) {  // xl 创建声卡的逻辑设备 
		snd_pcm_free(pcm);
		return err;
	}
	if (rpcm)
...
int snd_pcm_new(struct snd_card *card, const char *id, int device,
		int playback_count, int capture_count, struct snd_pcm **rpcm)            //   xl 1
{
...
static int snd_pcm_dev_register(struct snd_device *device)
{
	...
	for (cidx = 0; cidx < 2; cidx++) {
		...
		switch (cidx) {
		case SNDRV_PCM_STREAM_PLAYBACK:
			sprintf(str, "pcmC%iD%ip", pcm->card->number, pcm->device);   // xl  /dev/snd/name  
